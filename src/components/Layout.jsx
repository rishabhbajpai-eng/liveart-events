...
export const ContactFAB = () => {
  ...
  const channels = [
    ...
    { 
      ...
      href: 'https://pinterest.com/liveartevents', 
  ...
  return (
    <div ref={fabRef} className="fixed bottom-[24px] right-[24px] z-[9999] flex flex-col items-center">
          ...
          {isOpen && channels.map((channel, index) => (
            ...
            >
              ...
              <div className="absolute right-full mr-5 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none translate-x-4 group-hover:translate-x-0">
                <div className="glass-pill px-4 py-2 rounded-2xl text-xs font-black text-snow uppercase tracking-[0.2em] whitespace-nowrap border border-white/30 shadow-2xl">
                  {channel.label}
                </div>
              </div>
              ...
              <motion.a
                ...
                className={`w-[52px] h-[52px] rounded-2xl flex items-center justify-center text-white shadow-xl ${channel.shadow} transition-shadow duration-300 relative overflow-hidden group/btn`}
              >
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.92 }}
        className={`w-[68px] h-[68px] rounded-[28px] party-gradient text-snow flex items-center justify-center shadow-[0_20px_50px_rgba(0,206,209,0.4)] relative z-50 border-2 border-white/20 overflow-hidden group`}
      >
        {/* Inner Glow Surround */}
          ...
          {!isOpen && (
            <>
              <motion.span 
                ...
                className="absolute inset-0 rounded-[28px] bg-firozi/60 -z-10"
              ></motion.span>
              <motion.span 
                initial={{ scale: 1, opacity: 0.3 }}
                animate={{ scale: 1.4, opacity: 0 }}
                transition={{ duration: 2, delay: 0.5, repeat: Infinity, ease: 'easeOut' }}
                className="absolute inset-0 rounded-[28px] bg-ocean/40 -z-10"
              ></motion.span>
            </>
          )}
        </AnimatePresence>
